...
export default function ExploreComponent() {
    ...
    const fetchData = async () => {
        try {
            ...
            setLoadSupplements(response.data.result);

            const uniqueCategories = ["All", ...new Set(response.data.result.map((item) => item.category))];
    ...
    const PopupCard = ({product, onClose}) => (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-2xl shadow-xl w-[350px] p-6 relative">
                <button
                    onClick={onClose}
                ...
                <p className="text-center text-gray-500 mb-4">
                    {product.description || "Show off your best moves in this iconic sneaker."}
                </p>
                ...
                <p className="text-center text-2xl font-bold mb-4">${product.price || "110.99"}</p>
                ...
                <div className="flex justify-center items-center gap-1 mb-4 text-center">
                    <div className="flex items-center gap-1 bg-gray-100 px-3 py-1 rounded-lg shadow relative group">
                        <img src="https://img.icons8.com/?size=100&id=HxdvwPmtGaQL&format=png&color=000000"
                             alt="Verified Icon" className="w-6 h-6"/>
                        <span className="text-green-500 font-semibold text-sm">VERIFIED</span>
                        <span
                            className="absolute bottom-full mb-2 hidden group-hover:block bg-black text-white text-xs rounded-lg px-2 py-1">
            Verified by Power House Fitness Specialists
                ...
                <div className="flex justify-between items-center">
                    <button
                        ...
                        <HiOutlineShoppingCart size={20}/> Buy Now
                    </button>
            ...
            <div className="flex flex-wrap justify-center gap-3 mt-6">
                {categories.map((category) => (
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg ${activeCategory === category ? "bg-black text-white" : "bg-gray-100 text-gray-600"}`}
                    >
                        {category}
                    </button>
                ))}
                ...
                {filteredSupplements.map((supplement) => (
                    ...
                    >
                        <div className="relative flex justify-center">
                            <img
                                src={supplement.media || "https://via.placeholder.com/200"}
                                alt={supplement.name}
                                className="rounded-t-2xl h-[200px] object-cover"
                            />
                            <p className={`bg-white font-semibold rounded-lg p-1 text-sm absolute top-4 right-4 ${supplement.isAvailable ? "text-green-500" : "text-red-500"}`}>
                                {supplement.isAvailable ? "Available" : "Out of Stock"}
                            </p>
                        </div>
                        <div className="p-4">
