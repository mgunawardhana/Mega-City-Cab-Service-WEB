...
const Popup = ({ isOpen, onClose }) => {
    ...
    // Create timeline events and sort by step number
    const timelineEvents = guidelines
        .map((item) => ({
            icon: "📋",
            title: item.title,
            description: item.description,
            time: "2025-03-03 14:40:22",
            category: item.category,
            priority: item.priority,
            relatedTo: item.relatedTo || "Cab Service",
            stepNumber: parseInt(item.title.match(/Step (\d+)/)?.[1] || 0), // Extract step number
        }))
        .sort((a, b) => a.stepNumber - b.stepNumber); // Sort ascending by step number
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[1000]">
            <div className="bg-white rounded-lg p-6 w-[400px] max-h-[80vh] overflow-y-auto shadow-lg">
                ...
                {loading ? (
                    <p className="text-center text-gray-500">Loading guidelines...</p>
                ) : error ? (
                    ...
                    <div className="relative">
                        ...
                        {timelineEvents.length > 0 ? (
                            timelineEvents.map((event, index) => (
                                <div key={index} className="flex items-start mb-6 relative pl-10">
                                    <div className="absolute left-0 w-8 h-8 bg-amber-600 rounded-full flex items-center justify-center text-white text-xl">
                                        {event.icon}
                                    </div>
                                    <div className="ml-2">
                                        <p className="text-sm font-medium text-gray-800">{event.title}</p>
                                        <p className="text-xs text-gray-500">
                                            {event.description} • {event.category} • Priority: {event.priority} • Related to: {event.relatedTo}
                                        </p>
                            ...
                            <p className="text-center text-gray-500">No guidelines available.</p>
                        )}
                    </div>
                )}
            </div>
            ...
            <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-lg p-8">
                ...
                <div className="flex justify-center items-center mb-4">
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                        ...
                        />
                        <div className="w-11 h-6 bg-gray-200 rounded-full peer peer-focus:ring-4 peer-focus:ring-blue-300 peer-checked:bg-amber-500 transition-colors">
                ...
                <MapContainer center={position} zoom={13} style={{ height: "300px", width: "100%", zIndex: 10 }}>
